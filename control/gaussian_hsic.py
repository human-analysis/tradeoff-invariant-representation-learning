...
__all__ = ['GaussianHSICClassification']
...
class GaussianHSICClassification(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        sigma_s = self.rbfsigma(s, s.shape[0])
        H = torch.eye(n) - torch.ones(n) / n
        H = H.to(device=s.device)


        K_s = GaussianKernel(s.float(), s.float(), sigma_s)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        hsic = torch.trace(torch.mm(K_zm, K_sm)) / torch.sqrt(torch.trace(torch.mm(K_sm, K_sm)) * torch.trace(torch.mm(K_zm, K_zm)))
        # hsic = torch.trace(torch.mm(K_zm, K_sm)) / (n ** 2)
        self.log('val_hsic_loss', hsic, on_step=False, on_epoch=True)
        #################################################################
        # z, out = self.model(y)
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.hsic = torch.trace(torch.mm(K_zm, K_sm)) / torch.sqrt(torch.trace(torch.mm(K_sm, K_sm)) * torch.trace(torch.mm(K_zm, K_zm)))
        # hsic = torch.trace(torch.mm(K_zm, K_sm)) / (n ** 2)
        loss = (1 - self.opts.tau) * tgt_loss + self.opts.tau * self.hsic
        # loss = (1 - self.opts.tau) * tgt_loss - self.opts.tau * corr_zs
        ...
        if self.data_flag is not None:
            if self.data_flag:
                ...
                self.log('test_test_hsic_zs', self.hsic, on_step=False, on_epoch=True, prog_bar=True)
                ...
                self.log('test_train_hsic_zs', self.hsic, on_step=False, on_epoch=True, prog_bar=True)
    ...
    def test_epoch_end(self, outputs):
        ...
        if self.data_flag is not None:
            if self.data_flag:
                ...
                metrics_dict['test_test_hsic_zs'] = float(self.hsic.cpu().numpy().astype(np.float128))
                ...
                metrics_dict['test_train_hsic_zs'] = float(self.hsic.cpu().numpy().astype(np.float128))
    ...
    def configure_optimizers(self):
        ...
        if self.opts.scheduler_method is not None:
            ...
            return [optimizer_enc_tgt], [scheduler_enc_tgt]
        else:
            return [optimizer_enc_tgt]